...
const Completed = () => {
  ...
  const { data: completedNum, refetch: refetchCompletedNum } = useQuery({
    queryKey: ["completedNum", totalPages, currentPage],
    queryFn: async () => {
      const res = await axiosSecure.get(`/completed-num?email=${user?.email}`);
  ...
  const {
    ...
    queryKey: ["completedTasks", totalPages, currentPage],
    queryFn: async () => {
      const result = await axiosSecure.get(
        `/completedTasks?email=${user?.email}&skip=${currentPage * 4}`
      );
      return result.data;
    },
  });
  ...
  return (
    <div>
      ...
      <div className="mt-[15px]">
        ...
        <div
          className={`mt-10 ${completedNum?.total > 4 ? "block" : "hidden"}`}>
